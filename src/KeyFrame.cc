...
void KeyFrame::ProcessGlobalPositionMeas()
{
    ...
    //cout<<"keyframe id: "<<mnFrameId<<" globalPositionMeasBeforeProp size: "<<globalPositionMeasBeforeProp.size()<<endl;

    if(frameRelativePoses.empty())
    {
        cout<<"frameRelativePose empty, return!"<<endl;
        return;
    }
    ...
    {
        //frameRelativePoses为参考该关键帧的普通帧的相对于该关键帧的位姿，和普通帧的时间戳
        ...
        if(framePoseIt == frameRelativePoses.end())
        {
            if(mNextKF)
            {
                //cout<<"exixits nextKF"<<endl;
                //std::prev是一个标准库函数，用于获取一个迭代器的前一个位置。
                auto prevFrame = *(std::prev(framePoseIt));
                Sophus::SE3f TRelPrev = prevFrame.second;//获得该普通帧的前一个帧的位姿 Tf1r
                Sophus::SE3f TRelNext = mNextKF->GetPose() * GetPoseInverse();//获得该关键帧的下一个关键帧相对于该关键帧的位姿 Tf2r
                double tsPrev = prevFrame.first;//获得前一个帧的时间戳
                double tsNext = mNextKF->mTimeStamp;//获得当前关键帧后一个关键帧的时间戳
                double ratio = (tsNext - globalMeas->timestamp) / (tsNext - tsPrev);//获得当前gps时间在前一帧到后一个关键帧中的比例
                cout<<"ratio111: "<<ratio<<endl;
                Sophus::SE3f Tcir = Sophus::interpolate(TRelPrev, TRelNext, ratio);//进行插值，获得当前gps时间相对于该关键帧的位姿,
                globalPositionMeas.push_back(globalMeas);//当globalmeas具有相对于该关键帧的变换时，将其放入globalPositionMeas中
                posesRelativeToKF[globalMeas->id] = Tcir;//将globalMeas对应的相对于该关键帧的变换放入posesRelativeToKF中
                hasGlobalPositionMeasurement = true;
                pushed++;
    ...
    //std::cout<<"keframe id: "<< mnFrameId<<" timestamp: "<<mTimeStamp<<" globaltimestamp: "<<globalPositionMeas[0]->timestamp<<"pushed: "<<pushed<<endl;
    
}

void KeyFrame::ProcessGlobalPositionMeasExif(const GlobalPosition::GlobalPosition* meas)
{
    unique_lock<mutex> lock(mMutexGpsMeas);

    //cout<<"keyframe id: "<<mnFrameId<<" globalPositionMeasBeforeProp size: "<<globalPositionMeasBeforeProp.size()<<endl;
    
    if(mTimeStamp == meas->timestamp)
    {
        globalPositionMeas.push_back(meas);
        hasGlobalPositionMeasurement = true;
        Sophus::SE3f Tcir;
        posesRelativeToKF[meas->id] = Tcir;
        hasGlobalPositionMeasurement = true;
    }

    //std::cout<<"keframe id: "<< mnFrameId<<" timestamp: "<<mTimeStamp<<" globaltimestamp: "<<globalPositionMeas[0]->timestamp<<endl;
