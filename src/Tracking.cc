...
Tracking::Tracking(System *pSys, SPVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer,
    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL)), setGlobalPosOrigin(true), globalFrameAlignmentState(NO_MEAS_YET)
{
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::STEREO && !mpCamera2)
        mCurrentFrame = Frame(
            ...
            mThDepth,				// 远点,近点的区分阈值
            mpCamera,
            mpGlobalMeasCalib);				// 相机模型
    else if(mSensor == System::STEREO && mpCamera2)
        mCurrentFrame = Frame(mImGray,imGrayRight,timestamp,mpExtractorLeft,mpExtractorRight,mpSPVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpCamera2,mTlr,mpGlobalMeasCalib);
    else if(mSensor == System::IMU_STEREO && !mpCamera2)
    {
        mCurrentFrame = Frame(mImGray,imGrayRight,timestamp,mpExtractorLeft,mpExtractorRight,mpSPVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpGlobalMeasCalib,&mLastFrame,*mpImuCalib);
    ...
    else if(mSensor == System::IMU_STEREO && mpCamera2)
    {
        ...
        mCurrentFrame = Frame(mImGray,imGrayRight,timestamp,mpExtractorLeft,mpExtractorRight,mpSPVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpCamera2,mTlr,mpGlobalMeasCalib,&mLastFrame,*mpImuCalib);
 ...
 * Step 3 ：跟踪
 */
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im,   cv::Mat &im_ori, const double &timestamp, string filename)
{
    mImGray = im;
    //cout<<im.cols<<endl;
    // Step 1 ：将彩色图像转为灰度图像
    ...
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames)
            mCurrentFrame = Frame(mImGray, im_ori, timestamp,mpIniExtractor,mpSPVocabulary,mpCamera,mDistCoef,mbf,mThDepth,mpGlobalMeasCalib);
        else
            mCurrentFrame = Frame(mImGray, im_ori, timestamp,mpExtractorLeft,mpSPVocabulary,mpCamera,mDistCoef,mbf,mThDepth,mpGlobalMeasCalib);
    }

    if(mpSystem->settings_->bUseGlobalMeas_){
    ...
    else if(mSensor == System::IMU_MONOCULAR)
    {
        ...
        {
            mCurrentFrame = Frame(mImGray, im_ori,timestamp,mpIniExtractor, mpSPVocabulary,mpCamera,mDistCoef,mbf,mThDepth,mpGlobalMeasCalib,&mLastFrame,*mpImuCalib);
        }
        else
            mCurrentFrame = Frame(mImGray,im_ori,timestamp,mpIniExtractor,mpSPVocabulary,mpCamera,mDistCoef,mbf,mThDepth,mpGlobalMeasCalib,&mLastFrame,*mpImuCalib);
    }
    //cout<<"GrabImageMonocular empty: "<<mCurrentFrame.imgOri.empty()<<endl;
    // t0存储未初始化时的第1帧图像时间戳
...
void Tracking::GrabGpsData(const GlobalPosition::GlobalPosition* gpsMeasurement)
{
    ...
    //cout<<"time: " <<gpsMeasurement->timestamp<<"mpReferenceKF->mTimeStamp: "<<mpReferenceKF->mTimeStamp<<endl;
    
    if(mpReferenceKF)//主要是由于没有mpReferenceKF
    { 
       
        if(gpsMeasurement->timestamp >= mpReferenceKF->mTimeStamp)
        {
        
            mpReferenceKF->AddGlobalPositionMeas(gpsMeasurement);  
        }
    
        //std::cout<<"addglobal: "<<"gpsMeasurement->timestamp: "<<gpsMeasurement->timestamp<<"  mpReferenceKF->mTimeStamp"<< mpReferenceKF->mTimeStamp<<endl;
        //std::cout<<"addglobal: "<<"gpsMeasurement->timestamp: "<<gpsMeasurement->timestamp<<endl;
                                      
    }
    else
    {
        //std::cout<<"gpsMeasurement->timestamp: "<<gpsMeasurement->timestamp<<endl;
        //cout<<"mpReferenceKF->mTimeStamp: "<<mpReferenceKF->mTimeStamp<<endl;
        //std::cout<<"gpsMeasurement->timestamp: "<<gpsMeasurement->timestamp<<"  mpReferenceKF->mTimeStamp"<< mpReferenceKF->mTimeStamp<<endl;
        std::cout << "DROPPED GNSS MEASsss: " << gpsMeasurement->id << std::endl;
    }
    ...
    if((GetGlobalFrameAlignmentState() == FIRST_GLOBAL_MEAS_SET || GetGlobalFrameAlignmentState() == ALIGNING))
    {
...
void Tracking::Track()
{
...
#endif
        // IMU数据进行预积分
        //cout<<"track PreintegrateIMU begin"<<endl;
        PreintegrateIMU();
        //cout<<"track PreintegrateIMU end"<<endl;
#ifdef REGISTER_TIMES
    ...
    else
    {
        ...
        if(bOK || mState==RECENTLY_LOST)
        {
            ...
            if(bNeedKF && (bOK || (mInsertKFsLost && mState==RECENTLY_LOST &&
                ...
                //cout<<"mCurrentFrame size: "<<mCurrentFrame.imgOri.size()<<endl;
                //cout<<"createNewKeyFrame success!!"<<endl;

            //=====================往map2dfusion中发送当前帧的图片和位姿===========================
        //获取当前帧的图像！
            // if(bNeedKF){
            //     //cout<<"aassss"<<endl;
            //     // cv::Mat curImage = cv::Mat(mCurrentFrame.imgLeft.rows, mCurrentFrame.imgLeft.cols, CV_8UC1, mCurrentFrame.imgLeft.data);
            //     // cv::cvtColor(curImage, curImage, cv::COLOR_GRAY2BGR);
            //     if(!mCurrentFrame.imgOri.empty())
            //     {
            //         cv::Mat curImage = mCurrentFrame.imgOri.clone();
            //         //cout << "row/cols: "<<curImage.rows<<curImage.cols<<endl;
            //         Sophus::SE3f se3f = mCurrentFrame.GetPose().inverse();
            ...
            //         //cout<<"translation_vector_d: "<<translation_vector_d<<endl;
            //         std::pair<cv::Mat, Sophus::SE3d>  trans_frame(curImage, se3d);
            //         Trans.product(trans_frame);
            //         //cout<<"trans frame success!"<<endl;
            //     }
    ...
    if(mState==OK || mState==RECENTLY_LOST)
    {
        ...
        if(mCurrentFrame.isSet())
        {
            //Verbose::PrintMess("track: Tcr_ = mCurrentFrame.GetPose() * mCurrentFrame.mpReferenceKF->GetPoseInverse();", Verbose::VERBOSITY_QUIET);
            ...
            mCurrentFrame.mpReferenceKF->AddFrameRelativePose(mCurrentFrame.mTimeStamp, Tcr_);//增加参考关键帧的其他普通帧相对于该关键帧的位姿变换。
            //Verbose::PrintMess("track: AddFrameRelativePose(mCurrentFrame.mTimeStamp, Tcr_)", Verbose::VERBOSITY_QUIET);
        }
        else
        {
...
void Tracking::MonocularInitialization()
{
    ...
    else  // 第二帧来了
    {
        ...
        if ((mLastFrame.mnId-mInitialFrame.mnId > 30)||((int)mCurrentFrame.mvKeys.size()<=70)||((mSensor == System::IMU_MONOCULAR)&&(mLastFrame.mTimeStamp-mInitialFrame.mTimeStamp>1.0)))
        {
        ...
        if(mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn,mCurrentFrame.mvKeysUn,mvIniMatches,Tcw,mvIniP3D,vbTriangulated))
        {
            ...
            cout<<"mInitailaFrame mnId: "<<mInitialFrame.mnId<<endl;
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    cout<<"mvInip3d size: "<<mvIniP3D.size()<<endl;
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // Step 1：更新上一帧的位姿；对于双目或RGB-D相机，还会根据深度值生成临时地图点
    UpdateLastFrame();
    mCurrentFrame.globalPosition = currentPosition;
    
    // if(GetGlobalFrameAlignmentState() == Tracking::ALIGNED)
    // {
    //     const GlobalPosition::GlobalPosition *origin = mpReferenceKF->GetMap()->GetKFRelatedToGlobalOrigin()->GetGlobalPositionMeas()[0];
    //     const GlobalPosition::GlobalPosition *currentposition = currentPosition;
    //     const GlobalPosition::GlobalPosition *lastposition = lastPosition;
    //     Eigen::Vector3d relativeMeasurement = currentposition->getRelativeFromOrigin(*origin);
    //     Sophus::SE3f motionpose = mVelocity * mLastFrame.GetPose();
    //     Eigen::Matrix3f Rgw = mpReferenceKF->GetMap()->GetGlobalVIOAlignment();
    //     Eigen::Vector3d worldMeasurement = Rgw.inverse().cast<double>()*relativeMeasurement;
    //     Eigen::Quaternionf rotation = motionpose.unit_quaternion();
    //     Eigen::Vector3f newTranslation = worldMeasurement.cast<float>();
    //     Sophus::SE3f motionpose_gps = Sophus::SE3f(rotation, newTranslation);
    //     mCurrentFrame.SetPose(motionpose_gps);
    //     //const GlobalPosition::GlobalPosition *origin = mCurrentFrame->GetMap()->GetKFRelatedToGlobalOrigin()->GetGlobalPositionMeas()[0];
    // }
    ...
    else if(GetGlobalFrameAlignmentState() == Tracking::ALIGNED &&true)
    {
        const GlobalPosition::GlobalPosition *origin = mpReferenceKF->GetMap()->GetKFRelatedToGlobalOrigin()->GetGlobalPositionMeas()[0];
        const GlobalPosition::GlobalPosition *currentposition = currentPosition;
        const GlobalPosition::GlobalPosition *lastposition = lastPosition;
        Eigen::Matrix3f Rgw = mpReferenceKF->GetMap()->GetGlobalVIOAlignment();
        //试试把他转换成世界坐标系 tc1g0 tc2g0

        double scale = mpReferenceKF -> GetMap()->GetScaleVG();
        Eigen::Vector3f relativeMeasurement_end = Rgw.inverse()*currentposition->getRelativeFromOrigin(*origin).cast<float>();
        Eigen::Vector3f relativeMeasurement_begin = Rgw.inverse()*lastposition->getRelativeFromOrigin(*origin).cast<float>();

        Eigen::Vector3f relativeMeasurement_ed = relativeMeasurement_end-relativeMeasurement_begin;

        Eigen::Vector3f worldMeasurement = relativeMeasurement_ed * scale;
        Sophus::SE3f motionpose = mVelocity * mLastFrame.GetPose();

        Eigen::Vector3f motionpose_transrelative = motionpose.inverse().translation().transpose() - mLastFrame.GetPose().inverse().translation().transpose();
        
        Sophus::SE3f pose_gps = mVelocity * mLastFrame.GetPose();
        Sophus::SE3f pose_gps_inv = pose_gps.inverse();
        Eigen::Vector3f pose_gps_inv_tran_ = worldMeasurement+ mLastFrame.GetPose().inverse().translation();
        Sophus::SE3f motion_gps = Sophus::SE3f(motionpose.inverse().unit_quaternion(), pose_gps_inv_tran_.cast<float>()).inverse();
       
       
        //cout<<"motion pose transition: "<<motionpose.translation().transpose()<<endl;
        //cout<<"gps measurement: "<<worldMeasurement.transpose()<<endl;
       // cout<<"motion relative transition: "<<motion_gps.translation().transpose()<<endl;
        //缺少gps原点坐标以及尺度变换
        // Eigen::Quaternionf rotation = motionpose.unit_quaternion();
        // Eigen::Vector3f newTranslation = worldMeasurement.cast<float>();
        // Sophus::SE3f motionpose_gps = Sophus::SE3f(rotation, newTranslation);
        mCurrentFrame.SetPose(motion_gps);
    }
    else
    {
        ...
        string filename = "/media/xiao/data2/image_stitching/Rover-slam-gnss-exif/test_output.txt";
        std::ofstream outFile(filename, std::ios::app);
        outFile << "Frame's ID: " << mCurrentFrame.mnId
            << " translation: " << (mVelocity*mLastFrame.GetPose()).translation().transpose()
            << std::endl;
        outFile.close();
    }
    ...
    //  std::ofstream outFile(filename, std::ios::app);
    //  outFile << "Frame's ID: " << mCurrentFrame.mnId
    //         << " translation: " << mCurrentFrame.GetPose().translation().transpose()
    //         << std::endl;
    // outFile.close();
    //cout<<"Frame's ID: "<<mCurrentFrame.mnId<<"transtition: "<<mCurrentFrame.GetPose().translation().transpose()<<endl;
    ...
    else
    {
        ...
        else  // 如果积累的IMU数据量比较多，考虑使用IMU数据优化
        {
            ...
            if(!mbMapUpdated) //  && (mnMatchesInliers>30))
            {
                ...
                Verbose::PrintMess("TLM: PoseInertialOptimizationLastFrame success!!", Verbose::VERBOSITY_DEBUG);
            }
            else
            {
...
void Tracking::CreateNewKeyFrame()
{
    ...
    pKF->imgOri = mCurrentFrame.imgOri;
    // Step 2：将当前关键帧设置为当前帧的参考关键帧
    ...
    //cout<<"mpLocalMapper->InsertKeyFrame(pKF): "<<pKF->mnFrameId<<endl;
    // 插入好了，允许局部建图停止
    ...
    mpReferenceKF->ProcessGlobalPositionMeasExif(currentPosition);
    //cout<<"current aligentment state: "<< GetGlobalFrameAlignmentState()<<endl;
    if(setGlobalPosOrigin && mpReferenceKF->mPrevKF && mpReferenceKF->mPrevKF->HasGlobalPositionMeas())
    {
        ...
        std::cout << "SET FIRST GPS MEAS: " << mpReferenceKF->mPrevKF->mnFrameId <<std::endl;
        SetGlobalFrameAlignmentState(FIRST_GLOBAL_MEAS_SET);
    }
    pKF->globalPosition = mCurrentFrame.globalPosition;
    //cout<<mCurrentFrame.globalPosition<<endl;
    cout<<"create new keyframe success!: "<<pKF->mnId<<endl;
