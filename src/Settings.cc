...
Settings::Settings(const std::string &configFile, const int &sensor, const bool useGlobalMeas) :
    ...
    tbg_(Eigen::Vector3f::Zero()), gpsSigmaXY_(2.0f), gpsSigmaZ_(5.0f),
    globalAlignInitFrames_(80), globalAlignFinishMeasurements_(85)
{
    sensor_ = sensor;
...
void Settings::readGlobalMeasurements(cv::FileStorage &fSettings) 
{
    ...
    gpsSigmaXY_ = readParameter<float>(fSettings, "System.GpsSigmaXY", found, false);
    if(!found)
        gpsSigmaXY_ = 2.0f;

    gpsSigmaZ_ = readParameter<float>(fSettings, "System.GpsSigmaZ", found, false);
    if(!found)
        gpsSigmaZ_ = 5.0f;

    globalAlignInitFrames_ = readParameter<int>(fSettings, "System.GlobalAlignInitFrames", found, false);
    if(!found)
        globalAlignInitFrames_ = 80;

    globalAlignFinishMeasurements_ = readParameter<int>(fSettings, "System.GlobalAlignFinishMeasurements", found, false);
    if(!found)
        globalAlignFinishMeasurements_ = 85;

    cout<<"tbg_:ssss  "<<tbg_<<endl;
    cout<<"GNSS covariance sigma xy/z: "<<gpsSigmaXY_<<"/"<<gpsSigmaZ_<<endl;
    cout<<"GNSS alignment frames init/finish: "<<globalAlignInitFrames_<<"/"<<globalAlignFinishMeasurements_<<endl;
}
...
ostream &operator<<(std::ostream &output, const Settings &settings)
{
    ...
    if (settings.bUseGlobalMeas_)
    {
        output << "\t-GNSS covariance sigma xy/z: " << settings.gpsSigmaXY_
               << " / " << settings.gpsSigmaZ_ << endl;
        output << "\t-GNSS alignment frames init/finish: " << settings.globalAlignInitFrames_
               << " / " << settings.globalAlignFinishMeasurements_ << endl;
    }
