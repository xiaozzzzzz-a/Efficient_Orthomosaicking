...
void Optimizer::AlignmentRefinement(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool alignmentRotationFixed, bool bLarge, bool bRecInit)
{
    ...
    int vertexAlignmentId = maxKFid + 2000;
    VAlignment->setId(vertexAlignmentId);
    VAlignment->setFixed(false);
    optimizer.addVertex(VAlignment);

    double scalevg = pCurrentMap->GetScaleVG();
    cout<<"scalevg: "<<scalevg<<endl;
    VertexScale *VScale = new VertexScale(scalevg);

    int vertexScaledId = maxKFid + 3000;
    VScale->setId(vertexScaledId);
    VScale->setFixed(false);
    optimizer.addVertex(VScale);
    int edgeid = 0;
    // Set Local temporal KeyFrame vertices
    N=vpOptimizableKFs.size();
    cout<<"vpOtimizableKFs size: "<<N<<endl;
    for(int i=0; i<N; i++)
    {
        KeyFrame* pKFi = vpOptimizableKFs[i];

        VertexPose * VP = new VertexPose(pKFi);
        //cout<<"pKFi rotation: "<<pKFi->GetRotation()<<endl;
        VP->setId(pKFi->mnId);
        VP->setFixed(true);
        optimizer.addVertex(VP);

        //cout<<"pKFi->HasGlobalPositionMeas(): "<<pKFi->HasGlobalPositionMeas()<<endl;
        ...
        {       
            ...
            int size = pKFi->GetGlobalPositionMeas().size();
            //cout<<"pKFi's mid: "<<pKFi->mnFrameId<<" Local Inertial AlignemntRefinement size: "<<size<<endl; 
            
            const GlobalPosition::GlobalPosition* meas = pKFi->GetGlobalPositionMeas()[0];//获取里当前帧最近的地理全局位置
            const GlobalPosition::GlobalPosition* originMeas = kfRelatedToGpsOrigin->GetGlobalPositionMeas()[0];//获得gps起始点对应的关键帧的地理位置
            //kfRelatedToGpsOrigin->GetPoseRelativeToKF(originMeas->id)表示在originMeas->id的时间时，相对于参考KF的变换
            //这行代码就是获得对应gps时间的插值得到的位置相对于世界坐标系w的变换
            //Sophus::SE3f Twb0 = kfRelatedToGpsOrigin->GetPoseInverse() * kfRelatedToGpsOrigin->GetPoseRelativeToKF(originMeas->id).inverse() * kfRelatedToGpsOrigin->mImuCalib.mTcb;//gps初始时刻的body系与世界坐标系的位姿变换
            Sophus::SE3f Twb0 = kfRelatedToGpsOrigin->GetPoseInverse() * kfRelatedToGpsOrigin->GetPoseRelativeToKF(originMeas->id).inverse();

            
            //Eigen::Vector3f tbg = kfRelatedToGpsOrigin->mGlobalMeasCalib.tbg;
            
            Sophus::SE3f Tcir = pKFi->GetPoseRelativeToKF(meas->id);  //获得当前gps时刻相对于参考关键帧的位姿(通过两个帧的位姿插值计算出的)
            //cout<<"Tcir: "<<Tcir.matrix()<<endl;
            EdgeGlobalMeasurementVoScale* edge = new EdgeGlobalMeasurementVoScale(Twb0.cast<double>(), Tcir.cast<double>());
            edge->setVertex(0, VP);
            edge->setVertex(1, VAlignment);
            edge->setVertex(2, VScale);
            edge->setId(edgeid);
            edgeid++;
            Eigen::Vector3d relativeMeasurement = meas->getRelativeFromOrigin(*originMeas);

            //计算Edge Global measurement error: 
            Sophus::SE3d pose_se3(Eigen::Quaterniond(VP->estimate().Rcw[0]), VP->estimate().tcw[0]);
            Eigen::Vector3d translation = pose_se3.translation();
                //cout<<"translation vector3d: "<<translation.transpose()<<endl;
                Eigen::Quaterniond rotation(pose_se3.rotationMatrix());
                //cout<<"Quaterniond rotation: "<<rotation.coeffs().transpose()<<endl;
                Sophus::SE3d Trw(rotation, translation);
                Eigen::Matrix3d Rwc0 =Twb0.cast<double>().rotationMatrix();
                Eigen::Vector3d twc0 = Twb0.cast<double>().translation();
                Sophus::SE3d Twci_d = (Tcir.cast<double>() * Trw).inverse();
                Eigen::Vector3d estimatedPos(VAlignment->estimate() / VScale->estimate()  *((Trw.inverse().translation()) - twc0));
                Eigen::Vector3d _error = relativeMeasurement - estimatedPos;
                //cout<<"Aligment: ID: "<<pKFi->mnId<<" relativeMeasurement: "<<relativeMeasurement.transpose()<<" estimatedPos: "<<estimatedPos.transpose()<<" error: "<<_error.transpose()<<endl;
                string filename = "compareTraAlig.txt";
                ofstream outFile(filename, ios::app);
                outFile << "ID: " << pKFi->mnId<< " relativeMeasurement: " << relativeMeasurement.transpose()<< " estimatedPos: " << estimatedPos.transpose()<< endl;
                outFile.close();


            edge->setMeasurement(relativeMeasurement);//设置观测，该时刻gps位置相对于origin时刻gps位置的位姿变换
            //cout<<"meas's information: "<<meas->getCovariance().inverse().cast<double>();
            Eigen::Matrix3d information = meas->getCovariance().inverse().cast<double>();
                    // information(2 , 2) =0.6;
                    information(1 , 1) =2;
                    information(0 , 0) =1.2;
                    information(2 , 2) =2;
    ...
    VertexScale* estimatedScaleVG = static_cast<VertexScale*>(optimizer.vertex(vertexScaledId));
    pCurrentMap->SetScaleVG(estimatedScaleVG->estimate());
    //VertexScale* estimatedScale = static_cast<VertexScale*>(optimizer.vertex(vertexScaledId));
    cout<<"update scale: "<<estimatedScaleVG->estimate()<<endl;
    //delete VAlignment;
}
...
void Optimizer::LocalBundleAdjustmentWithGlobalMeas(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int &num_fixedKF, int &num_OptKF, int &num_MPs, int &num_edges)
{
    // 该优化函数用于LocalMapping线程的局部BA优化
    // Local KeyFrames: First Breath Search from Current Keyframe
    list<KeyFrame *> lLocalKeyFrames;
    ...
    for (int i = 0, iend = vNeighKFs.size(); i < iend; i++)
    {
        ...
        // 记录局部优化id，该数为不断变化，数值等于局部化的关键帧的id，该id用于防止重复添加
        pKFi->mnBALocalForKF = pKF->mnId;
        if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap && pKFi->mnId != 0)
            lLocalKeyFrames.push_back(pKFi);
    }
    ...
    cout<<"num_fixedKF: "<<num_fixedKF<<endl;
    // Setup optimizer
    // 步骤5：构造g2o优化器
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType *linearSolver;
    ...
    const int vertexAlignmentId = 100000000;
    VAlignment->setId(vertexAlignmentId);
    bool alignmentRotationFixed = true;
    ...
    int origEdges = 0;
    Eigen::Vector3d sumGlobalMeasError = Eigen::Vector3d::Zero();
    double sumGlobalMeasErrorNorm = 0.0;
    // */

    // DEBUG LBA
    pCurrentMap->msOptKFs.clear();
    pCurrentMap->msFixedKFs.clear();
    double scale = pCurrentMap->GetScaleVG();
    VertexScale *VScale = new VertexScale(scale);
    const int vertexScaledId = 100000001;
    VScale->setId(vertexScaledId);
    VScale->setFixed(true);
    optimizer.addVertex(VScale);
    ...
    for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end(); lit != lend; lit++)
    {
        ...
        if(pKFi->mnId == 0)
            continue;
        // VertexPose * VP = new VertexPose(pKFi);
        // VP->setId(pKFi->mnId+1000);
        // cout<<"pKFi->mnId: "<<pKFi->mnId<<endl;
        // VP->setFixed(false);
        // optimizer.addVertex(VP);
        
        g2o::VertexSE3Expmap *vSE3 = new g2o::VertexSE3Expmap();
        Sophus::SE3<float> Tcw = Sophus::SE3<float>();
        Tcw = pKFi->GetPose();
        //std::cout << "Tcw as 4x4 matrix:\n" << Tcw.matrix() << std::endl;
        ...
        if(kfRelatedToGpsOrigin && kfRelatedToGpsOrigin->HasGlobalPositionMeas() && kfRelatedToGpsOrigin->mGlobalMeasCalib.mbIsSet && pKFi->HasGlobalPositionMeas())
        {
            ...
            for(const GlobalPosition::GlobalPosition* meas : pKFi->GetGlobalPositionMeas())
            {
                ...
                Sophus::SE3f Twg0 = kfRelatedToGpsOrigin->GetPoseInverse();//gps坐标系源点所对应的关键帧在世界坐标系中的位姿。

                EdgeGlobalMeasurementVo* edge = new EdgeGlobalMeasurementVo(Twg0.cast<double>());

                edge->setVertex(0, vSE3);//当前帧在世界坐标系下的位子
                edge->setVertex(1, VAlignment);//slam坐标系和gps坐标系的旋转
                edge->setVertex(2, VScale);//slam坐标系与gps坐标系的尺度变换
                Eigen::Vector3d relativeMeasurement = meas->getRelativeFromOrigin(*originMeas);//在gps坐标系下，当前帧相对于原点的位置变换。

                g2o::SE3Quat pose_se3 = vSE3->estimate();//前关键帧在 SLAM 坐标系下的估计位姿 
                Eigen::Vector3d translation = pose_se3.translation();
                Eigen::Quaterniond rotation = pose_se3.rotation();        //cout<<"Quaterniond rotation: "<<rotation.coeffs().transpose()<<endl;
                Sophus::SE3d Trw(rotation, translation);
                Sophus::SE3d Twg0_d(Twg0.cast<double>());
                Eigen::Vector3d estimatedPos(VAlignment->estimate() / VScale->estimate()  *((Trw.inverse().translation()) - Twg0_d.translation()));
                Eigen::Vector3d _error = relativeMeasurement - estimatedPos;
                
                double measurementNorm = relativeMeasurement.norm();
                if(measurementNorm > 1e-3 &&measurementNorm < 1e5)
                {
                    //cout<< "measurementNorm's norm: "<<measurementNorm<<endl;
                    edge->setMeasurement(relativeMeasurement);
                    Eigen::Matrix3d information = meas->getCovariance().inverse().cast<double>();
                    edge->setInformation(information);
                    sumGlobalMeasError += _error;
                    sumGlobalMeasErrorNorm += _error.norm();

                    g2o::RobustKernelHuber *rk = new g2o::RobustKernelHuber;
                    edge->setRobustKernel(rk);
                    int thHuberMono = 7.815;
                    rk->setDelta(thHuberMono);
    ...
    cout<<"optimized gps frame's num: "<<num<<endl;
    if(edgesMeasCount > 0)
    {
        cout << "LocalBundleAdjustmentWithGlobalMeas mean GNSS _error: "
             << (sumGlobalMeasError / edgesMeasCount).transpose()
             << " mean norm: " << (sumGlobalMeasErrorNorm / edgesMeasCount)
             << " edges: " << edgesMeasCount << endl;
    }
    else
    {
        cout << "LocalBundleAdjustmentWithGlobalMeas mean GNSS _error: no GNSS edges" << endl;
    }
    num_OptKF = lLocalKeyFrames.size();
    ...
    float err = optimizer.activeRobustChi2();
    //cout<<"acitvteRobustChi2 err: "<<err<<endl;
...
void Optimizer::LocalInertialBAWithGlobalMeas(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool alignmentRotationFixed, bool bLarge, bool bRecInit)
{
    ...
    vpOptimizableKFs.push_back(pKF);
    pKF->mnBALocalForKF = pKF->mnId;
    for(int i=1; i<Nd; i++)//将相邻距离小于maxOpt的关键帧作为可优化关键帧。
    {
        if(vpOptimizableKFs.back()->mPrevKF)
        {
            vpOptimizableKFs.push_back(vpOptimizableKFs.back()->mPrevKF);
        ...
        for(vector<MapPoint*>::iterator vit=vpMPs.begin(), vend=vpMPs.end(); vit!=vend; vit++)
        {
            ...
            if(pMP)
                if(!pMP->isBad())
                    if(pMP->mnBALocalForKF!=pKF->mnId)
                    {
                        lLocalMapPoints.push_back(pMP);//将可优化关键帧的地图点加入lLocalMapPoints中
                        pMP->mnBALocalForKF=pKF->mnId;
                    }
        }
    }
    ...
    if(vpOptimizableKFs.back()->mPrevKF)
    {
        ...
        vpOptimizableKFs.back()->mPrevKF->mnBAFixedForKF=pKF->mnId;//如果可优化关键帧有上一阵，则将上一帧加入固定关键帧中
    }
    else
    {
        vpOptimizableKFs.back()->mnBALocalForKF=0;
        vpOptimizableKFs.back()->mnBAFixedForKF=pKF->mnId;
        lFixedKeyFrames.push_back(vpOptimizableKFs.back());//如果该可优化关键帧没有上一帧，则将该帧变为固定关键帧
    ...
    for(int i=0, iend=vpNeighsKFs.size(); i<iend; i++)
    {
        ...
        if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
        {
            lpOptVisKFs.push_back(pKFi);//与当前帧有共视关系的关键帧
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,tuple<int,int>>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(pKFi->mnBALocalForKF!=pKF->mnId && pKFi->mnBAFixedForKF!=pKF->mnId)
            {
                ...
                if(!pKFi->isBad())
                {
                    lFixedKeyFrames.push_back(pKFi);//将共识地图点的共视关键帧再加入到固定关键帧集中。
                    break;
                }
            }
        }
    ...
    for(int i=0; i<N; i++)
    {
        ...
        {                      
            ...
            {               
                const GlobalPosition::GlobalPosition* originMeas = kfRelatedToGpsOrigin->GetGlobalPositionMeas()[0];
                //kfRelatedToGpsOrigin：Twg_0 kfRelatedToGpsOrigin:Tc0 g0 
