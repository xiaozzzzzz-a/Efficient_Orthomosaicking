    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    node = fsSettings["visualmapping"];
    
    if(!node.empty())
    {
        mVisualmapping = static_cast<int>(fsSettings["visualmapping"]) != 0;
    }
    ...
    cout<<"tracking create success!"<<endl;
    //Initialize the Local Mapping thread and launch
    //创建并开启local mapping线程
    mpLocalMapper = new LocalMapping(this, mpAtlas, mSensor==MONOCULAR || mSensor==IMU_MONOCULAR,
                                     mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO || mSensor==IMU_RGBD, strSequence);
    cout<<"localmapping create success!"<<endl;
    ...
    // 创建并开启闭环线程
    cout<<"localclosing create success!"<<endl;
    ...
    cout<<"localclosing create success111!"<<endl;
    //usleep(10*1000*1000);
    ...
    cout<<"localclosing create success11122!"<<endl;
    // Fix verbosity
    // 打印输出中间的信息，设置为安静模式
 ...
 */
Sophus::SE3f System::TrackMonocular(const cv::Mat &im,  cv::Mat &im_ori, const double &timestamp, const vector<IMU::Point>& vImuMeas, const GlobalPosition::GlobalPosition* pMeas, string filename)
{
    ...
    // for(size_t i_g = 0; i_g < vMeas.size(); i_g++)
    //     mpTracker->GrabGpsData(vMeas[i_g]);
    mpTracker->GrabGpsData(pMeas);
    mpTracker->currentPosition = pMeas;
    // 计算相机位姿
    //cout<<"mpTracker->GrabImageMonocular(imToFeed,timestamp,filename);"<<endl;
    //cout<<"GrabImageMonocular empty: "<<im_ori.empty()<<endl;

    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(imToFeed, im_ori, timestamp, filename);
    mpTracker->lastPosition = pMeas;
    // 更新跟踪状态和参数
    unique_lock<mutex> lock2(mMutexState);
    mTrackingState = mpTracker->mState;
    mTrackedMapPoints = mpTracker->mCurrentFrame.mvpMapPoints;
...
void System::SaveKeyFrameTrajectoryTUM(const string &filename)
{
    ...
    f.open(filename.c_str());
    f << fixed;
    cout<<"vpKF's size: "<<vpKFs.size()<<endl;
    ...
    cout<<"saving keyframe trajectory to "<<filename.c_str()<<endl;
