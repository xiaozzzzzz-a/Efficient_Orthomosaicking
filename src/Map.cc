...
void Map::ApplyScaledRotation(const Sophus::SE3f &T, const float s, const bool bScaledVel)
{
    ...
    for (set<KeyFrame *>::iterator sit = mspKeyFrames.begin(); sit != mspKeyFrames.end(); sit++)
    {
        ...
        //cout<<pKF->mnId<<"apply scaled rotation transition: "<<Tcy.translation().transpose()<<endl;
        // 更新关键帧速度
    ...
    for (set<MapPoint *>::iterator sit = mspMapPoints.begin(); sit != mspMapPoints.end(); sit++)
    {
        ...
        // if(!ransac.is_finished())
        // {
        //     cv::Point3d p3d= cv::Point3d(static_cast<double>(pMP->GetWorldPos()[0]), static_cast<double>(pMP->GetWorldPos()[1]), static_cast<double>(pMP->GetWorldPos()[2]));
        //     if (!ransac.is_finished())
        //     {
...
// void Map::ApplySim3Transform(Eigen::Matrix4f T, const bool bScaledVel)
// {
//     unique_lock<mutex> lock(mMutexMap);

//     // Body position (IMU) of first keyframe is fixed to (0,0,0)
//     Eigen::Matrix3f Rg0w = T.block<3,3>(0,0);

//     Eigen::Vector3f tg0w = T.topRightCorner<3,1>();

//     //Eigen::Mat

//     float scale = std::cbrt(Rg0w.determinant());
...
//          */
//         KeyFrame *pKF = *sit;
//         Eigen::Matrix4f Twc = pKF->GetPoseInverse().matrix().cast<float>();
//         Eigen::Matrix4f Tg0c = T*Twc;
...
void Map::SetScaleVG(double scaleVG)
{
    unique_lock<mutex> lock(mMutexMap);
    scalevg = scaleVG;
}

double Map::GetScaleVG()
{
    unique_lock<mutex> lock(mMutexMap);
    return scalevg;
}
