...
public:
    ...
    Frame(const cv::Mat &imGray,  cv::Mat &im_ori, const double &timeStamp, SPextractor* extractor,SPVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, const GlobalPosition::Calib &GlobalMeasCalib,Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
    // Destructor
    // ~Frame();
    ...
    const GlobalPosition::GlobalPosition* globalPosition;
...
public:
    ...
    cv::Mat imgLeft, imgRight, imgOri;
