...
public:
    ...
    virtual void oplusImpl(const double* update_){
        //cout<<this->id()<<" update rortation: "<<this->_id<<" "<<update_[0]<<" "<<update_[1]<<" "<<update_[2]<<endl;
...
public:
    ...
    virtual void oplusImpl(const double *update_)
    {
        //cout<<"scale update: "<<*update_<<endl;
...
public:
    ...
    EdgeSE3ExpmapPrior(Eigen::Matrix3d Rgw1): Rgw(Rgw1)
    {
    ...
    void computeError()
    {
        ...
        Eigen::Matrix3d rotation = Rgw.inverse();
        
        //Eigen::Matrix3d rotation = Eigen::AngleAxisd(M_PI, Eigen::Vector3d::UnitX()).toRotationMatrix();
        Eigen::Vector3d translation(0.0, 0.0, 0.0);
        g2o::SE3Quat Tw1c = v->estimate().inverse() * g2o::SE3Quat(rotation,translation);
        g2o::SE3Quat err = _measurement * Tw1c;
        //cout<<"_measurement Tw1c_se2: "<<_measurement.inverse().translation().transpose();
        //cout<<"Tw1c: "<<Tw1c.translation().transpose()<<endl;
        //g2o::SE3Quat err = _measurement * v->estimate().inverse();
        _error = err.log();
        //cout<<"error: "<<_error<<endl;
        //cout<<"plane Error success ";
    }
...
public:
    ...
    void computeError(){
        ...
        // Eigen::Vector3d estimatedPos(vRotation->estimate() * s *((Twbi * tbg) - (Rwb0 * tbg + twb0)));//论文中公式6
        // _error = _measurement - estimatedPos;
        // cout<<"Edge Global measurement error: "<<_error.transpose()<<endl;
        ...
        Sophus::SE3d Trw(Eigen::Quaterniond(vPose->estimate().Rcw[0]), vPose->estimate().tcw[0]);
        //Sophus::SE3d Tbc(Eigen::Quaterniond(vPose->estimate().Rbc[0]), vPose->estimate().tbc[0]);
        Sophus::SE3d Twbi = (Tcir * Trw).inverse();
        Eigen::Vector3d estimatedPos(vRotation->estimate() * s *((Twbi * tbg) - (Rwb0 * tbg + twb0)));//论文中公式6
        _error = _measurement - estimatedPos;
        //cout<<"_measurement: "<<_measurement.transpose()<<" _estimatedPos: "<<estimatedPos.transpose()<<" Edge Global measurement error: "<<_error.transpose()<<endl;
    }
...
// class EdgeGlobalMeasurementVoScale : public g2o::BaseMultiEdge<3, Eigen::Vector3d>
// {

// public:
//     EIGEN_MAKE_ALIGNED_OPERATOR_NEW

//     EdgeGlobalMeasurementVoScale(Sophus::SE3d Twb0_, Sophus::SE3d Tcir_, Eigen::Vector3d tbg_):Twb0(Twb0_),Tcir(Tcir_),tbg(tbg_){resize(3);}
//     virtual bool read(std::istream& is){return false;}
//     virtual bool write(std::ostream& os) const{return false;}
//     void computeError(){
//         const VertexPose* vPose = static_cast<const VertexPose*>(_vertices[0]);
...
//         Sophus::SE3d Trw(Eigen::Quaterniond(vPose->estimate().Rcw[0]), vPose->estimate().tcw[0]);
//         Sophus::SE3d Twbi = (Tcir * Trw).inverse();
//         Eigen::Vector3d estimatedPos(vRotation->estimate() / vScale->estimate() *((Twbi * tbg) - (Rwb0 * tbg + twb0)));
//         cout<<this->id()<<" _measurement: "<<_measurement.transpose()<<endl;
//         _error = _measurement - estimatedPos;
//         cout<<"align refine scale "<<vScale->estimate()<<" _estimatedPos: "<<estimatedPos.transpose()<<" Edge Global measurement error: "<<_error.transpose()<<endl;
...
public:
    ...
    EdgeGlobalMeasurementVoScale(Sophus::SE3d Twc0_, Sophus::SE3d Tcir_):Twc0(Twc0_),Tcir(Tcir_){resize(3);}
    virtual bool read(std::istream& is){return false;}
    virtual bool write(std::ostream& os) const{return false;}
    void computeError(){
        const VertexPose* vPose = static_cast<const VertexPose*>(_vertices[0]);
        ...
        Eigen::Matrix3d Rwc0 = Twc0.rotationMatrix();
        Eigen::Vector3d twc0 = Twc0.translation();
        Sophus::SE3d Trw(Eigen::Quaterniond(vPose->estimate().Rcw[0]), vPose->estimate().tcw[0]);
        Sophus::SE3d Twci = (Tcir * Trw).inverse();
        Eigen::Vector3d estimatedPos(vRotation->estimate() / vScale->estimate() *((Twci.translation()) - (twc0)));
        //cout<<this->id()<<" _measurement: "<<_measurement.transpose()<<endl;
        _error = _measurement - estimatedPos;
        //cout<<"align measurement: "<<_measurement.transpose()<<" etimatedPos: "<<estimatedPos.transpose()<<" Edge Global measurement error: "<<_error.transpose()<<endl;

    }
    public: 
        Sophus::SE3d Twc0, Tcir;
};

class EdgeGlobalMeasurementVo : public g2o::BaseMultiEdge<3,Eigen::Vector3d>
{
//g2o::VertexSE3Expmap,VertexRotation
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    
    EdgeGlobalMeasurementVo(Sophus::SE3d Twb0_):Twc0(Twb0_){resize(3);} //Twc0为gnss原点关键帧在slam坐标系中的变换
    virtual bool read(std::istream& is){return false;}
    virtual bool write(std::ostream& os) const{return false;}
    //virtual void linearizeOplus();
    void computeError(){
        ...
        const VertexRotation* vRotation = static_cast<const VertexRotation*>(_vertices[1]);
        const VertexScale* vScale = static_cast<const VertexScale*>(_vertices[2]);
        g2o::SE3Quat pose_se3 = vPose->estimate();//前关键帧在 SLAM 坐标系下的估计位姿 
        Eigen::Vector3d translation = pose_se3.translation();
        //cout<<"translation vector3d: "<<translation.transpose()<<endl;
        Eigen::Quaterniond rotation = pose_se3.rotation();
       ...
       // Eigen::Vector3d estimatedPos(vRotation->estimate() * vScale->estimate() *((Twbi * tbg) - (Rwb0 * tbg + twb0)));//论文中公式6
       
        //Eigen::Vector3d estimatedPos(vRotation->estimate() / vScale->estimate()  *((Twci.translation()) - twc0));
        Eigen::Vector3d estimatedPos(vRotation->estimate() / vScale->estimate()  *((Trw.inverse().translation()) - twc0));
        _error = _measurement - estimatedPos;

        // std::cout << "local ba EstimatedPos: " << estimatedPos.transpose()
        //   << " | GPSMeas: " << _measurement.transpose()
        //   << " | Error: " << _error.transpose() << std::endl;
        // cout<<"Edge Global measurement error: "<<_error.transpose()<<endl;
    }
...
class EdgeGlobalMeasurementVoAfterTransform : public g2o::BaseMultiEdge<3,Eigen::Vector3d>
{
//g2o::VertexSE3Expmap,VertexRotation
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    
    EdgeGlobalMeasurementVoAfterTransform(){resize(3);}
    virtual bool read(std::istream& is){return false;}
    virtual bool write(std::ostream& os) const{return false;}
    //virtual void linearizeOplus();
    void computeError(){
        ...
        const VertexRotation* vRotation = static_cast<const VertexRotation*>(_vertices[1]);
        const VertexScale* vScale = static_cast<const VertexScale*>(_vertices[2]);
        g2o::SE3Quat pose_se3 = vPose->estimate();
        Eigen::Vector3d translation = pose_se3.translation();
        //cout<<"translation vector3d: "<<translation.transpose()<<endl;
        Eigen::Quaterniond rotation = pose_se3.rotation();
        //cout<<"Quaterniond rotation: "<<rotation.coeffs().transpose()<<endl;
        Sophus::SE3d Trw(rotation, translation);
        Eigen::Vector3d estimatedPos(Trw.inverse().translation());
        _error = _measurement - estimatedPos;
        //cout<<"Edge Global measurement error: "<<_error.transpose()<<endl;
    }
